...
/* ── PartnerLogo render ──────────────────────── */
/*
 * Borderless partner item:
 * - keeps the company logo
 * - shows the company name directly beside/below it
 * - no card, border, background, shadow, or rounded box
 *
 * The infinite-scroll animation is intentionally left to the parent
 * component, since this file does not contain the scrolling container.
 */
export function PartnerLogo({ name, logo, Logo, mark }) {
  const [failed, setFailed] = useState(false);

  return (
    <div className="flex shrink-0 items-center gap-3 whitespace-nowrap bg-transparent border-0 p-0 m-0 shadow-none">
      {Logo ? (
        <Logo />
      ) : mark ? (
        <span className="text-[20px] font-extrabold tracking-[-0.04em] text-slate-800">
          {mark}
        </span>
      ) : failed ? null : (
        <img
